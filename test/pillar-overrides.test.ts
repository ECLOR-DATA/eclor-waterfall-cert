...
describe("planComparisonColumns — explicit synth-comparison column plan", () => {
  ...
  test("aggregate (1.5.0.0): a hidden segment can leave ONE aggregate column", () => {
    expect(
      kinds(planComparisonColumns(3, 2, [true, true, false], [false, false, true]))
    ).toEqual(["P0", "b0.0", "b0.1", "P1", "A1", "P2"]);
    expect(
      kinds(planComparisonColumns(3, 2, [true, false, true], [false, true, false]))
    ).toEqual(["P0", "A0", "P1", "b1.0", "b1.1", "P2"]);
  });

  test("aggregate is inert on a SHOWN segment and absent flags keep 'remove'", () => {
    expect(planComparisonColumns(3, 2, [true, true, true], [true, true, true])).toEqual(
      planComparisonColumns(3, 2)
    );
    expect(planComparisonColumns(3, 2, [true, true, false], [])).toEqual(
      planComparisonColumns(3, 2, [true, true, false])
    );
    expect(kinds(planComparisonColumns(2, 1, [false, true], [true, false]))).toEqual([
      "P0",
      "b0.0",
      "P1"
    ]);
